...
bool QtLocalPeer::isClient()
{
    ...
    //如果lockFile 写锁失败，则表示已经设置了写锁，即已经有程序实例启动了
    //则当前对象为启动的客户端client，返回true
...
bool QtLocalPeer::sendMessage(const QString &message, int timeout, bool block)
{
    ...
    res &= socket.waitForReadyRead(timeout); // wait for ack 向server实例发送完数据之后，等待对方ack确认
...
void QtLocalPeer::receiveConnection()
{
    ...
    QByteArray uMsg;
    quint32 remaining;
    ds >> remaining;   //获取的第一个uint32为数据长度？？
